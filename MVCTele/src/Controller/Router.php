...
class Router
{
    ...
    private $programmationController;
    ...
    public function __construct(){
        ...
        $this->programmationController = new ProgrammationController();
    }
    ...
    public function routeRequest(){
        ...
        } elseif ($this->isController('programme')){ 

            if ($this->isAction('fetchAll')){
                $this->programmeController->fetchAllAction();
            }
            if($this->isAction('del')){
                $this->programmeController->deleteAction();
            }
            if($this->isAction('add')){
                $this->programmeController->addOrModifyAction();
            }
            if($this->isAction('modify')){
                $this->programmeController->addOrModifyAction();
            }
            if($this->isAction('detail')){
                $this->programmeController->detailAction();
            }
            if($this->isAction('type')){
                $this->programmeController->typeAction();
            }
        } elseif ($this->isController('programmation')){ 

            if ($this->isAction('fetchAll')){
                $this->programmationController->fetchAllAction();
            }
            if($this->isAction('del')){
                $this->programmationController->deleteAction();
            }
            if($this->isAction('add')){
                $this->programmationController->addOrModifyAction();
            }
            if($this->isAction('modify')){
                $this->programmationController->addOrModifyAction();
            }
            if($this->isAction('detail')){
                $this->programmationController->detailAction();
            }
        }
        
        else{
            $this->defaultController->indexAction();
            //$this->defaultController->typeProgrammeAction();
        }
    }
